...
static void build_path(char fpath[PATH_MAX], const char *path)
{
    ...
    snprintf(normal, PATH_MAX, "%s%s", real_root, path);
    snprintf(enc, PATH_MAX, "%s%s.enc", real_root, path);
                       ...
                       enum fuse_readdir_flags flags)
{
    ...
    // Jika file .enc → hilangkan extensi
    if (strstr(name, ".enc")) {
                      ...
                      struct fuse_file_info *fi)
{
    ...
    snprintf(fpath, PATH_MAX, "%s%s.enc", real_root, path);
